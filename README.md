...
Open the frontend and the login page offers **Create the first
account** — a fresh deployment has no accounts, so this is the one
moment an account can be made without signing in. After that, accounts
live in the **Users** tab and every one of them has full access. Walk
through onboarding and you're trading.
...
# Credentials for scripts/ and the AI backtest engine. These name an
# ordinary account created in the Users tab -- no account is
# provisioned from this file. The first one is created through the
# login page's "Create the first account", which the backend allows
