...
# The schema enumerates every endpoint and parameter — useful for development,
# an attack surface map for production.
...
_CODE_FINGERPRINT_FILES = (
    ...
    "swing_trader/refdata_build.py",
    "swing_trader/refdata_sync.py",
    "swing_trader/backtest_bars.py",
    "db/schema.py",
...
def api_update_auth_user(
    ...
    bumps the row's token_version in ``update_user``, so every token minted
    against the old password stops authenticating -- including the caller's
...
def api_learning_permissions(conn=Depends(conn_dependency), current_user: dict = Depends(get_current_user)):
    """The action-class x rung permission matrix and the document allowlist."""
    return _run(action_learning_permissions, conn)
