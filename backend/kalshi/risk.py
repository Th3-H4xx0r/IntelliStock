...
class RiskCaps:
    ...
    model_only_size_mult: float = 1.0       # size haircut for no-sharp (model-only) bets (1.0 = off)
    order_size_min_cents: int = 0           # UI "order size" RANGE floor ($ per order; 0/0 = auto/Kelly)
    order_size_max_cents: int = 0           # range ceiling — bot sizes within [min,max] by edge conviction
