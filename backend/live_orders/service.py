...
class LiveOrderService:
    ...
    def submit(self, intent: OrderIntent) -> OrderSubmission:
        ...
        existing = self.lifecycle_store.get(intent.idempotency_key)
        if existing is not None:
            if existing.intent != intent:
                return _denial(intent, "idempotency.identity_drift")
        ...
        try:
            ...
            try:
                self._append_internal(
                    ...
                    reason=f"{type(exc).__name__}: transport outcome unknown",
                )
            except Exception:
                pass
            return OrderSubmission(
