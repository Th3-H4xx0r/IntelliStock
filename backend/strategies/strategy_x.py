...
_BEAR_DD_PEAK_KEY = "_sx_bear_dd_peak"
_BEAR_DD_HALTED_KEY = "_sx_bear_dd_halted"
    ...
    def run_once(self, symbols, prices, current_time, config, conditions,
        ...
        if bear_mode in {"shadow", "active"}:
            ...
            regime = advance_regime_state(
                ...
                config=cfg)
            # The account's own high-water mark, applied AFTER the tape-based
            # ladder and able only to reduce exposure. Every other control here
            # reads the tape; this is the one that notices when the tape's
            # signals were all correct and the position lost money anyway.
            guard = drawdown_guard(
                nav, peak=cache.get(_BEAR_DD_PEAK_KEY),
                halted=cache.get(_BEAR_DD_HALTED_KEY, False), config=cfg,
                rearm_ok=bool(sig.risk_on))
            if guard.scale < 1.0 and regime.risk_fraction > 0:
                regime = RegimeDecision(
                    regime.state, round(regime.risk_fraction * guard.scale, 6),
                    regime.confirm_kind, regime.confirm_count,
                    regime.observation_id,
                    f"{regime.reason} | {guard.reason}")
            if guard.halted and regime.risk_fraction > 0:
                regime = RegimeDecision(
                    "defensive", 0.0, regime.confirm_kind, regime.confirm_count,
                    regime.observation_id, guard.reason)
            ...
            cache[_BEAR_DD_PEAK_KEY] = guard.peak
            cache[_BEAR_DD_HALTED_KEY] = guard.halted
            ...
            cache["_sx_bear_shadow"] = {
                ...
                "regime": {
                    ...
                    "drawdown": {"peak": guard.peak, "halted": guard.halted,
