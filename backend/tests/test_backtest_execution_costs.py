...
from backend.backtest_summary import (
    ...
    assert_execution_provenance_promotable,
...
def test_complete_execution_provenance_passes_the_promotion_boundary():
    simulator = NextEventExecutionSimulator(COSTS)
    emulator = PortfolioEmulator(
        10_000.0,
        execution_simulator=simulator,
    ...
    emulator.process_quote(_quote(T1))
    emulator.save_portfolio_snapshot({"SPY": 100.0}, timestamp=T1)
    summary = compute_backtest_summary(
        emulator,
        emulator.get_portfolio_history(),
        10_000.0,
    )

    assert_execution_provenance_promotable(summary)
    assert summary["execution_promotion_eligible"] is True


def test_malformed_fill_provenance_fails_closed():
    summary = {
        "execution_provenance_complete": True,
        "execution_cost_model_version": COSTS.version,
        "execution_cost_model": COSTS.as_dict(),
        "total_fees": 0.0,
        "spread_cost": 0.0,
        "slippage_cost": 0.0,
        "unfilled_order_count": 0,
        "rejected_order_count": 0,
        "fill_provenance": [{"order_id": "missing-most-fields"}],
    }

    with pytest.raises(ExecutionProvenanceError, match="fill_provenance"):
        assert_execution_provenance_promotable(summary)
