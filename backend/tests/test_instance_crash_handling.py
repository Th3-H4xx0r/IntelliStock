...
def test_operator_stop_requested(monkeypatch):
    fake_r = MagicMock()
    run = fake_r.db.return_value.table.return_value.get.return_value.run
    monkeypatch.setattr(inst, "r", fake_r)
    run.return_value = {"runCommand": False}
    assert inst._operator_stop_requested(MagicMock(), "id") is True
    run.return_value = {"runCommand": True}
    assert inst._operator_stop_requested(MagicMock(), "id") is False
    run.return_value = {}  # field absent defaults to running
    assert inst._operator_stop_requested(MagicMock(), "id") is False
    run.return_value = None  # no doc
    assert inst._operator_stop_requested(MagicMock(), "id") is False
    run.side_effect = RuntimeError("db down")  # DB error never ends the block
    assert inst._operator_stop_requested(MagicMock(), "id") is False
...
def _feed_always_errors(monkeypatch):
    fake_r = MagicMock()
    (fake_r.db.return_value.table.return_value.get.return_value
        .changes.return_value.run.side_effect) = RuntimeError("feed boom")
    monkeypatch.setattr(inst, "r", fake_r)
    monkeypatch.setattr(inst, "get_conn", lambda: MagicMock())
...
def test_changefeed_kalshi_exits_immediately(monkeypatch):
    ...
    monkeypatch.setattr(inst.os, "_exit", _exit_recorder(exits))
    with pytest.raises(_Exit):
        inst.terminate_thread_on_command(MagicMock())
...
def test_changefeed_kalshi_graceful_close_returns_quietly(monkeypatch):
    ...
    fake_r = MagicMock()
    (fake_r.db.return_value.table.return_value.get.return_value
        .changes.return_value.run.return_value) = iter([])  # closes with no events
    monkeypatch.setattr(inst, "r", fake_r)
    trips = []
    monkeypatch.setattr(inst, "trip_crash", lambda *a, **k: trips.append(a))
    ...
    monkeypatch.setattr(inst.os, "_exit", _no_exit)
    inst.terminate_thread_on_command(MagicMock())  # returns quietly
    assert trips == []
...
def test_changefeed_equities_trips_after_retries(monkeypatch):
    ...
    monkeypatch.setattr(inst.os, "_exit", _no_exit)
    inst.terminate_thread_on_command(MagicMock())  # returns after tripping
    assert len(trips) == 1
...
def test_operator_stop_still_exits_never_trips(monkeypatch):
    ...
    fake_r = MagicMock()
    (fake_r.db.return_value.table.return_value.get.return_value
        .changes.return_value.run.return_value) = iter([{"new_val": {"runCommand": False}}])
    monkeypatch.setattr(inst, "r", fake_r)
    exits = []
    monkeypatch.setattr(inst.os, "_exit", _exit_recorder(exits))
    trips = []
