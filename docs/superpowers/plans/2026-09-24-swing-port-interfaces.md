...
- The live swing entry hint has no share count: `buy_cash = max(min(equity × 0.125, budget) × size_adjustment, prior close)`; the engine buys `floor(buy_cash / live price)`. Approval orders carry `qty` (`approvals.build_approved_order`), capped at the same budget (§6; Final round, I-1).
  ...
  - **Final round (seams m1, as ruled):** when the collateral cannot be read, the scan still decides and sends that tick's exits (RSI > 70 and the close-based stop and target), which are risk-reducing and never depend on collateral. It plans no entries and does not latch, so the entries retry next tick. A retried scan does not repeat an exit's SELL notice within the session. Before the final round the lane planned no entries and still latched the session.
