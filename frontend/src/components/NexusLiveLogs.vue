...
// ── Config ────────────────────────────────────────────────────────────────────
// Polling cadence: running builds get 2s, completed/idle builds get 15s so we
// still notice when the next build starts without thrashing the API.
...
async function fetchLogs(sinceLine, gen) {
  ...
  const res = await fetch(
    `${API_BASE}/nexus-graph-builds/latest/logs?since_line=${sinceLine}`,
...
async function downloadFullLog() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/nexus-graph-builds/latest/logs?since_line=0`,
      { headers: authHeaders() },
    )
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    const data = await res.json()
    const banner = data.truncated
      ? `# WARNING: server truncated log at 5000 lines. Total in source: ${data.total_lines}. For full log, read the file on the api container: /app/nexus_graph_logs/build_${data.id}.log\n`
      : ''
    ...
    a.href = url
    a.download = `nexus-${data.id || 'latest'}${data.truncated ? '-first5k' : ''}.log`
    ...
    const a    = document.createElement('a')
    a.href = url
    a.download = `nexus-${buildId.value || 'latest'}${droppedCount.value > 0 ? '-partial' : ''}.log`
    document.body.appendChild(a)
    a.click()
    a.remove()
    URL.revokeObjectURL(url)
...
const shortBuildId = computed(() => {
  if (!buildId.value) return 'latest'
...
const friendlyStatus = computed(() => {
  ...
  if (s === 'none') return 'No builds yet'
  return 'Unknown'
})
    ...
    <div v-if="open" class="bg-[#0d1117] border-t border-[#21262d] relative">
      ...
      >
        <template v-if="status === 'none'">
          No builds yet. Start the Nexus engine above to begin a build.
        </template>
