...
/// Terminal-style live-log panel for an instance.
///
/// Uses [LogTailer] with `pathBuilder: (n) => '/instances/$instanceId/live-logs?since_line=$n'`.
/// Auto-sticks to the bottom unless the user scrolls up.
/// Exported for reuse by the live_trading feature.
class LiveLogsPanel extends ConsumerStatefulWidget {
  const LiveLogsPanel({super.key, required this.instanceId});

  final String instanceId;

  @override
  ConsumerState<LiveLogsPanel> createState() => _LiveLogsPanelState();
}
...
class _LiveLogsPanelState extends ConsumerState<LiveLogsPanel> {
  ...
  void _buildTailer() {
    _tailer = LogTailer(
      ...
      pathBuilder: (n) =>
          '/instances/${widget.instanceId}/live-logs?since_line=$n',
  ...
  void _togglePause() {
    ...
    // Check if currently paused via stream state (we drive pause/resume directly)
    setState(() {});
    // We'll use a flag on the tailer
  ...
  Color get _statusColor {
    ...
    if (s == 'completed') return AppColors.textFaint;
    return AppColors.textFaint;
  }

  bool get _statusPulsing =>
      (_tailerState.finalStatus ?? '').toLowerCase() == 'running';
  ...
  String get _shortId {
    final id = _tailerState.buildId ?? widget.instanceId;
    final s = id.toString();
  ...
  // ── Search field (shown inside header when open + has lines) ────────────────
  ...
  Widget _buildLogArea() {
    ...
    if (_tailerState.lines.isEmpty) {
      ...
      String msg = 'Waiting for log output…';
      if (s == 'none') {
        msg = 'Broker not running. Start the instance to begin live trading logs.';
      } else if (_tailerState.error != null) {
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                      color: AppColors.stroke(AppColors.info)),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
